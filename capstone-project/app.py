...
def load_model():
    try:
        # Try multiple file formats and methods
        model = None
        scaler = None
        model_name = None
        
        # Method 1: Try joblib first (more stable)
        try:
            import joblib
            model = joblib.load('model_obesitas_optimal.joblib')
            scaler = joblib.load('scaler.joblib')
            model_name = "Model Obesitas Optimal (joblib)"
            return model, scaler, model_name
        except FileNotFoundError:
            pass
        except Exception as e:
            st.warning(f"Joblib loading failed: {e}")
        
        # Method 2: Try pickle with optimal model
        try:
        ...
        if model is not None:
            try:
                ...
                try:
                    ...
                    raise FileNotFoundError("Scaler file tidak ditemukan")
        ...
        st.error("📁 File yang dibutuhkan (salah satu):")
        st.error("• model_obesitas_optimal.pkl/joblib ATAU random_forest_model.pkl")
        st.error("• scaler.pkl/joblib")
        st.info("💡 Coba re-upload file atau buat ulang dengan format yang berbeda.")
        st.code("""
# Di Google Colab, coba buat dengan joblib:
import joblib
joblib.dump(model, 'model_obesitas_optimal.joblib')
joblib.dump(scaler, 'scaler.joblib')
        ...
        st.error("1. Re-create .pkl files with lower pickle protocol")
        st.error("2. Use joblib instead of pickle")
        st.error("3. Check file corruption")
        return None, None, None
